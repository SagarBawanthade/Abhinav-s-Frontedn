...
const ManageOrders = () => {
  const [orders, setOrders] = useState([]); // Correct type for orders
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [selectedOrder, setSelectedOrder] = useState(null);  // Track selected order with null
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get("https://backend.abhinavsofficial.com/api/order/orders");
  ...
  const handleDelete = async () => {
    if (selectedOrder) {
      try {
        await axios.delete(`https://backend.abhinavsofficial.com/api/order/delete-order/${selectedOrder._id}`);
